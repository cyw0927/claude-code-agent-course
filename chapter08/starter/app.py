...
def search_tasks(items, keyword):
    ...
    if not clean_keyword:
        return None
    return [task for task in items if clean_keyword in task["title"].lower()]
...
def main():
    ...
    command = sys.argv[1]
    if command == "list":
        print_tasks(tasks); return
    if command == "add":
        if len(sys.argv) != 3:
            print("오류: add 명령에는 제목이 필요합니다."); return
        task = add_task(tasks, sys.argv[2])
        if task is None:
            print("오류: Task 제목은 비어 있을 수 없습니다."); return
        print(f"Task {task['id']}를 추가했습니다."); print_tasks(tasks); return
    if command == "complete":
        if len(sys.argv) != 3:
            print("오류: complete 명령에는 Task ID가 필요합니다."); return
        task_id = parse_task_id(sys.argv[2])
        if task_id is None:
            print("오류: Task ID는 숫자여야 합니다."); return
        if not complete_task(tasks, task_id):
            print(f"오류: ID {task_id}인 Task를 찾을 수 없습니다."); return
        print(f"Task {task_id}를 완료 처리했습니다."); print_tasks(tasks); return
    if command == "due":
        if len(sys.argv) != 4:
            print("오류: due 명령에는 Task ID와 날짜가 필요합니다."); return
        task_id = parse_task_id(sys.argv[2])
        if task_id is None:
            print("오류: Task ID는 숫자여야 합니다."); return
        result = set_due_date(tasks, task_id, sys.argv[3])
        if result == "INVALID_DATE":
            print("오류: 날짜는 YYYY-MM-DD 형식의 실제 날짜여야 합니다."); return
        if result == "NOT_FOUND":
            print(f"오류: ID {task_id}인 Task를 찾을 수 없습니다."); return
        print(f"Task {task_id}의 마감일을 {sys.argv[3]}로 설정했습니다."); print_tasks(tasks); return
    if command == "priority":
        if len(sys.argv) != 4:
            print("오류: priority 명령에는 Task ID와 우선순위가 필요합니다."); return
        task_id = parse_task_id(sys.argv[2])
        if task_id is None:
            print("오류: Task ID는 숫자여야 합니다."); return
        result = set_priority(tasks, task_id, sys.argv[3])
        if result == "INVALID_PRIORITY":
            print("오류: priority는 low, medium, high 중 하나여야 합니다."); return
        if result == "NOT_FOUND":
            print(f"오류: ID {task_id}인 Task를 찾을 수 없습니다."); return
        print(f"Task {task_id}의 priority를 {sys.argv[3].lower()}로 설정했습니다."); print_tasks(tasks); return
    if command == "filter":
        if len(sys.argv) != 3:
            print("오류: filter 명령에는 done 또는 pending 값이 필요합니다."); return
        filtered = filter_tasks(tasks, sys.argv[2])
        if filtered is None:
            print("오류: filter는 done 또는 pending 중 하나여야 합니다."); return
        print_tasks(filtered); return
    if command == "search":
        if len(sys.argv) != 3:
            print("오류: search 명령에는 검색어가 필요합니다."); return
        found = search_tasks(tasks, sys.argv[2])
        if found is None:
            print("오류: 검색어는 비어 있을 수 없습니다."); return
        if not found:
            print("검색 결과가 없습니다."); return
        print_tasks(found); return
