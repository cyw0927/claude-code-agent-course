...
- STEP 11~16: Markdown 보고서, Slack/Gmail 모듈, `src/` 함수화, `main.py` 통합,
  샘플 8건을 사용한 로컬 전체 실행
- 고용24 채용정보 Open API 연결: 공식 `wanted` XML 필드를 데이터 명세에 매핑
...
- 고용24 XML 파서와 공식 필드 매핑은 오프라인 fixture로 검사 완료

## 사용자 확인 또는 키가 필요한 항목

- STEP 10: Notebook의 `verification_df`에서 Gemini 결과에 대한 사람의 최종 판단
- 고용24 실호출: `WORK24_API_KEY`를 넣은 뒤 응답과 공고 수 확인
...
- 필수: `WORK24_API_KEY`, `GEMINI_API_KEY`
- 선택: `SLACK_WEBHOOK_URL`, `GMAIL_USER`, `GMAIL_APP_PASSWORD`

크롤링은 구현하지 않았다. 잡코리아/사람인 대신 공식 고용24 Open API를 사용한다.
