...
def main():
    ...
    if command == "search":
        if len(sys.argv) != 3:
            print("오류: search 명령에는 검색어가 필요합니다.")
            return
        found = search_tasks(tasks, sys.argv[2])
        if found is None:
            print("오류: 검색어는 비어 있을 수 없습니다.")
            return
        if not found:
            print("검색 결과가 없습니다.")
            return
    ...
    if command == "stats":
        ...
        for key in ("total", "completed", "pending", "high_priority"):
            print(f"{key}: {stats[key]}")
        return
    if command == "export":
        print(tasks_to_csv(tasks), end="")
        return
